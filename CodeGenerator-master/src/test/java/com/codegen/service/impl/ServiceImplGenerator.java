...
public class ServiceImplGenerator extends CodeGeneratorManager implements CodeGenerator {
    ...
    public void genCode(String tableName) {
        ...
        try {
            ...
            // 创建 Service 接口
            File serviceFile = new File(PROJECT_PATH + JAVA_PATH + PACKAGE_PATH_SERVICEIMPL + customMapping
    ...
    public static List<String> getServiceMethods(String mapperJavaFilePath, String modelName) throws IOException {
        ...
        for (String content : contentList) {
            ...
            if (match) {
                ...
                methodContent.append(StringUtils.FOUR_SPACES).append(StringUtils.FOUR_SPACES).append(StringUtils.FOUR_SPACES);
                if("selectByCondition".equals(methodName)){
                    methodContent.append("PageParams pageParams = new PageParams();\n" +
                            "                pageParams.setUseFlag(useFlag);\n" +
                            "                pageParams.setPage(page);\n" +
                            "                pageParams.setPageSize(pageSize);\n" +
                            "                map.put(\"page\", pageParams);\n");
                }
                methodContent.append("            return ").append(mapperName).append("." + methodName).append("(");
